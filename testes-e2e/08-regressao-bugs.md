...
- **Verificação:** `flutter analyze` limpo nos 3 arquivos tocados; varredura `grep` por `FilePicker.platform.pickFiles`/`.single.path`/`File(...path)` no restante do projeto não encontrou mais ocorrências do padrão problemático (fora de um caso de crop/ajuste de imagem em `step1_professional_data_page.dart:441`, que é um refator maior — visualização de imagem via `Image.file` — fora do escopo desta correção; ver observação abaixo).
...
- **Observação (não corrigida, escopo maior):** `step1_professional_data_page.dart:441` ainda constrói `File(pickedFile.path)` a partir de `image_picker` para abrir uma tela de ajuste/crop de imagem (`_showImageAdjustmentSheet`). Não foi corrigido nesta sessão porque a tela de ajuste provavelmente usa `Image.file(File)` para preview, o que exigiria um refator mais amplo (crop cross-platform com bytes) — não apenas trocar a leitura do arquivo. Recomenda-se tratar como item separado.
...
### REG-20 — Chat da consulta falha silenciosamente em consulta com data retroativa (NOVO)
- **Severidade:** Média
...
- **Causa raiz:** não identificada com certeza — suspeita de alguma condição no código de envio de chat (`chat_service.dart` ou equivalente) ligada à data/hora da consulta (ex.: alguma checagem de janela de horário ou validação de "consulta em andamento" que falha silenciosamente para consultas com data passada).
- **Impacto:** perda silenciosa de mensagens em consultas específicas — paciente/médico acreditam ter enviado, mas a mensagem nunca chega ao outro lado.
- **Ação recomendada:** investigar o método de envio de `chat_mensagens` (`chat_service.dart`) por alguma validação implícita de data/hora; reproduzir com uma consulta criada com data passada de propósito.
- **Ref.:** MED-09 em `03-medico-mobile.md`

### REG-21 — Listas de consultas do médico não atualizam após finalizar atendimento (NOVO)
- **Severidade:** Baixa (cosmético)
- **Ambiente:** Médico mobile — `/appointment` (abas "Próximas consultas" e "Histórico")
- **Descrição:** após finalizar um atendimento (MED-12) com sucesso — status migrado para `finalizada` no banco, confirmado via SQL — a aba "Próximas consultas" continua mostrando a consulta finalizada com o estado antigo "Em andamento", e a aba "Histórico" mostra "Nenhum atendimento no histórico" até um reload completo da página. Após reload manual, tudo aparece correto (consulta em "Realizada" no histórico).
- **Causa raiz:** provável falta de invalidação/refetch do estado local das listas (`_all` em `appointments_page.dart`) após a ação de finalizar — o app não recarrega os dados nem atualiza otimisticamente o item finalizado.
- **Impacto:** UX confusa logo após finalizar (medico vê estado desatualizado), mas o dado no backend está correto — não é perda de dado, só falta de refresh de UI.
- **Ação recomendada:** chamar `_load()` (ou atualização otimista do item na lista) imediatamente após a chamada de finalizar atendimento ter sucesso.
- **Ref.:** MED-12 em `03-medico-mobile.md`

### REG-22 — Rota `/home` (médico) acessível por paciente sem guarda de papel (achado incidental)
- **Severidade:** Baixa
- **Ambiente:** Mobile — roteador (`app_router.dart`)
- **Descrição:** ao navegar manualmente para `/home` (rota do dashboard do médico) estando logado como paciente, a tela renderiza o shell do dashboard do médico ("Boas vindas, Dr(a). Teste!") em vez de redirecionar/bloquear o acesso. Nenhum dado de outro usuário vaza (contadores ficam zerados por falta de linha em `medicos` para esse `user_id`), mas confirma a ausência de guarda de rota por papel já apontada em SEC-08.
- **Impacto:** confusão de UX, não é vazamento de dado (RLS protege os dados mesmo com a tela abrindo).
